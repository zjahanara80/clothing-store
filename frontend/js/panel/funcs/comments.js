    ...
    comments.forEach((comment, index) => {
        ...
        if (comment.parent == null) {
           ...
           <tr>
            ...
            <td>
                <button type='button' class='btn btn-primary show-btn' onclick='showCommentFunc(${JSON.stringify(comment.title)} ,${JSON.stringify(comment.message)} , ${JSON.stringify(comment.rating)})'>مشاهده</button>
            </td>

            <td>
               ${comment.approved ? 
                `<button type='button' class='btn btn-danger delete-btn' onclick="rejectComment('${comment._id}' , ${comment.isVerifiedUser})">رد</button>`
              :  
               `
                <button type='button' class='btn btn-success delete-btn' onclick="acceptComment('${comment._id}' , ${comment.isVerifiedUser})">تایید</button>
               `} 
            </td>
            <td>
...
const showCommentFunc = (commentTitle,commentText,commentScore) => {
    Swal.fire({
        title: commentTitle,
        text : commentText,
